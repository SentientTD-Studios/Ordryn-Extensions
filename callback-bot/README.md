...
Optional: turn on the project's inbound webhook. With this extension enabled, `POST /api/v1/webhooks/inbound` also accepts `complete` and `set_field` (plus the usual `create` / `comment` flags).

## Sample relay

`relay/main.go` receives the JSON event, then calls `POST /api/v1/ext/callback` with the token from the payload. On `task.due_soon` it posts a comment. On `task.created` it sets `callback-bot.review_by`.
